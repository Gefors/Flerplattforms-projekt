...
function GeneratedWorkout({ workoutPlan }: GeneratedWorkoutProps) {
        ...
        <div className="rounded-xl overflow-hidden w-full mt-10 shadow-lg mb-10">
            ...
            {workoutPlan && (
                <div className="mx-auto bg-white  overflow-hidden w-full p-8">
                    ...
                    <div className="grid gap-4 mt-4 grid-cols-[repeat(auto-fit,minmax(400px,1fr))]">
                        {workoutPlan.exercises.map((exercise, index) => (
                            <div key={index} className="flex flex-row h-auto w-full p-4 rounded-lg shadow-lg items-start border border-zinc-100">
                                <div className="flex flex-col w-1/2 gap-4 p-2">
                                    <h5 className="font-semibold">{index + 1} - {exercise.name}</h5>
                                ...
                                <div className="flex flex-row p-2 h-full gap-2 w-1/2 rounded-lg mt-2 items-start justify-center bg-zinc-50 overflow-hidden">
                                    <MuscleGroupFront muscleActivations={displayMuscle(exercise)} />
                                    <MuscleGroupBack muscleActivations={displayMuscle(exercise)} />
                                </div>
                            </div>
                        ))}
                    </div>

                    <Button className="bg-blue-500 p-2 text-white rounded-lg shadow-lg mt-10 cursor-pointer" onClick={saveWorkout}>
                        Save Workout
                    </Button>
                </div>
            )}
